...
namespace Buscaminas
{
    public class MineSweeper : IGameDelegate
    {
        ...
        void IGameDelegate.OnDraw(GameDelegateEvent gameEvent, ICanvas canvas)
        {
            ...
            int bombCount = 10;
            board.Init(cellCountWith, cellCountHeigh, bombCount);
            RenderCells(canvas, board);
            ...
            {
                for (int i = 0; i < cellCountWith; i++)
                {
                    for (int j = 0; j < cellCountHeigh; j++)
                    {
                        if (board.IsOpen(i, j))
                        { 
                            if (board.IsBombAt(i, j))
                            {
                            ...
                            {
                                int proximity = board.GetBombProximity(i, j);
                                canvas.FillShader.SetColor(new rgba_f64(0.5, 0, 0.1, 1));
                                canvas.Transform.SetTranslation(i, j);
                                canvas.DrawRectangle(new rect2d_f64(0.1, 0.1, 0.8, 0.8));
                            }
                        }
                        else if (board.IsFlagAt(i, j))
                        {
                            canvas.FillShader.SetColor(PickColor(i, j, board));
                            canvas.Transform.SetTranslation(i, j);
                            canvas.DrawRectangle(new rect2d_f64(0.1, 0.1, 0.8, 0.8));
